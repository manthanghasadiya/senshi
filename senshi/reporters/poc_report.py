...
def _generate_python_poc(findings: list[Finding], target: str, cookie: str) -> str:
    ...
    lines = [
        ...
        '',
        'def color(text, c):',
        '    return COLORS.get(c, "") + str(text) + COLORS["NC"]',
        '',
        'def get_session():',
        '    s = requests.Session()',
        '    s.verify = False',
        ...
        '    sep = "=" * 50',
        '    print("\\n" + sep)',
        '    print(" Senshi PoC Verification")',
        '    print(f" {len(POCS)} findings to verify")',
        '    print(sep + "\\n")',
        '',
        '    for i, poc in enumerate(POCS, 1):',
        '        sev = poc["severity"]',
        '        sev_color = {"CRITICAL": "RED", "HIGH": "RED", "MEDIUM": "YELLOW", "LOW": "CYAN"}.get(sev, "NC")',
        '        prefix = color(f"[{sev}]", sev_color)',
        '        print(f"{prefix} Finding {i}: {poc[\'title\']}")',
        '        print(f"  Endpoint: {poc[\'method\']} {poc[\'endpoint\']}")',
        '        print(f"  Payload:  {str(poc.get(\'payload\', \'\'))[:60]}")',
        '        print()',
        '',
        '        try:',
        '            if poc["method"] == "GET":',
        ...
        '                    v_tag = color("[VERIFIED]", "GREEN")',
        '                    print(f"  {v_tag} Evidence found: {poc[\'evidence_pattern\'][:50]}")',
        '                    verified = True',
        '                else:',
        '                    nf_tag = color("[NOT FOUND]", "YELLOW")',
        '                    print(f"  {nf_tag} Expected evidence not in response")',
        '',
        ...
        '            err_tag = color("[ERROR]", "RED")',
        '            print(f"  {err_tag} Request failed: {e}")',
